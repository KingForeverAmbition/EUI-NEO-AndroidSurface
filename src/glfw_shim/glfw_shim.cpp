...
namespace
{
    ...
    GLFWvidmode g_videoMode = {0, 0, 8, 8, 8, 120};
...
GLFWwindow *glfwCreateWindow(int width, int height, const char *, GLFWmonitor *, GLFWwindow *)
{
    ...
    w->egl.width = (g_surfaceWidth > 0) ? g_surfaceWidth : (width > 0 ? width : native_window_screen_x);
    w->egl.height = (g_surfaceHeight > 0) ? g_surfaceHeight : (height > 0 ? height : native_window_screen_y);
    ...
    const EGLint cfgAttr[] = {
        EGL_SURFACE_TYPE, EGL_WINDOW_BIT, EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT | EGL_OPENGL_ES3_BIT, EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_ALPHA_SIZE, 8, EGL_NONE};
    EGLint nCfg;
    EGLConfig cfg;
    ...
    __android_log_print(ANDROID_LOG_INFO, "EUI", "chosen EGLConfig id=%d renderable=0x%x", cfgId, cfgRenderable);
    ...
    __android_log_print(ANDROID_LOG_INFO, "EUI", "EGL context ready, GL_VERSION=%s", (const char *)glGetString(GL_VERSION));
...
{
    if (w) w->shouldClose = (v != 0);
}
void glfwSetWindowTitle(GLFWwindow *, const char *) {}
void glfwSetWindowUserPointer(GLFWwindow *w, void *p)
{
    if (w) w->userPointer = p;
}
...
void glfwGetWindowSize(GLFWwindow *w, int *width, int *height)
{
    if (w)
    {
        if (width) *width = w->egl.width;
        if (height) *height = w->egl.height;
    }
}
...
void glfwGetCursorPos(GLFWwindow *w, double *x, double *y)
{
    if (w)
    {
        if (x) *x = w->input.cursorX;
        if (y) *y = w->input.cursorY;
    }
}
...
{
    if (!w) return nullptr;
    auto old = w->mouseButtonCallback;
...
bool eui_android_exit_requested()
{
    return AndroidBridge::instance().exitRequested.load();
}

bool eui_android_surface_lost()
{
    return AndroidBridge::instance().surfaceLost.load();
}

void eui_android_release_egl(GLFWwindow *w)
{
    if (!w) return;
...
bool eui_android_promote_pending_window()
{
    auto &bridge = AndroidBridge::instance();
    std::lock_guard<std::mutex> lock(bridge.surfaceMutex);
...
void eui_android_poll_ime_frame()
{
    auto &bridge = AndroidBridge::instance();
...
bool eui_android_wait_for_surface()
{
    auto &bridge = AndroidBridge::instance();
...
bool eui_android_acquire_egl(GLFWwindow *w)
{
...
const char *eui_android_text_font_path()
{
...
const char *eui_android_icon_font_path()
{
    static std::string path;
    const auto &dir = AndroidBridge::instance().fontsDir;
    if (!dir.empty())
...
const char *eui_android_files_dir()
{
    const auto &dir = AndroidBridge::instance().fontsDir;
...
extern "C"
{
    ...
    Java_com_eui_neo_EuiActivity_nativeMain(JNIEnv *env, jobject thiz, jobject surface, jobject assetManager)
    {
        auto &bridge = AndroidBridge::instance();
        {
            std::lock_guard<std::mutex> lock(bridge.surfaceMutex);
    ...
    Java_com_eui_neo_EuiActivity_nativeResize(JNIEnv *, jobject, jint w, jint h)
    {
        auto &bridge = AndroidBridge::instance();
        std::lock_guard<std::mutex> lock(bridge.surfaceMutex);
        bridge.pendingWidth = w;
        bridge.pendingHeight = h;
        bridge.hasPendingResize.store(true);
    }

    JNIEXPORT void JNICALL
    Java_com_eui_neo_EuiActivity_nativeTouchEvent(JNIEnv *, jobject, jfloat x, jfloat y, jint action)
    {
        auto &bridge = AndroidBridge::instance();
        std::lock_guard<std::mutex> lock(bridge.touchMutex);
...
// ══════════════════════════════════════════════════════════════════
// NDK 可执行文件专用接口（替代 Kotlin Activity + JNI）
// 允许从纯 C++ main() 直接把 ANativeWindow 和触摸事件注入进来
// ══════════════════════════════════════════════════════════════════

void eui_android_set_native_window(ANativeWindow *nw, int sw, int sh)
{
    g_surfaceWidth = sw;
    g_surfaceHeight = sh;
    g_videoMode.width = sw;
    g_videoMode.height = sh;
    auto &bridge = AndroidBridge::instance();
    {
        std::lock_guard<std::mutex> lk(bridge.surfaceMutex);
        bridge.nativeWindow = nw;
        bridge.surfaceLost.store(false);
        bridge.surfaceReady.store(true);
        bridge.exitRequested.store(false);
        bridge.imeVisible.store(false);
        // No JVM / activityRef in NDK-executable mode
    }
    bridge.surfaceCv.notify_all();
}
...
{
    auto &bridge = AndroidBridge::instance();
    std::lock_guard<std::mutex> lk(bridge.touchMutex);
    bridge.touchQueue.push_back({x, y, action});
}

void eui_android_set_font_paths(const char *textFont,
                                const char *iconFont,
                                const char *filesDir)
{
    auto &bridge = AndroidBridge::instance();
    // Store directly in fontsDir so eui_android_text_font_path() works
    // We stash all three as: fontsDir = filesDir, then override the static paths
    if (filesDir) bridge.fontsDir = filesDir;
    // Override the path helpers to return fixed paths
    // (simplest: just use the fontsDir to re-derive the paths the same way)
    (void)textFont;
    (void)iconFont;
    // If caller passes absolute paths, put them directly
    if (textFont && textFont[0] == '/')
    {
        // extract dir from textFont path
        std::string p(textFont);
        auto pos = p.rfind('/');
        if (pos != std::string::npos)
        {
            bridge.fontsDir = p.substr(0, pos);
        }
    }
}
void eui_android_request_exit()
{
    AndroidBridge::instance().exitRequested.store(true);
    AndroidBridge::instance().surfaceCv.notify_all();
}