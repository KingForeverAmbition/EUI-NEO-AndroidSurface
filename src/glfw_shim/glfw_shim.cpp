...
extern "C" void eui_android_set_native_window(ANativeWindow* nw, int sw, int sh) {
    g_surfaceWidth  = sw;
    g_surfaceHeight = sh;
...
extern "C" void eui_android_inject_touch(float x, float y, int action) {
    auto& bridge = AndroidBridge::instance();
    std::lock_guard<std::mutex> lk(bridge.touchMutex);
    bridge.touchQueue.push_back({x, y, action});
}

extern "C" void eui_android_set_font_paths(const char* textFont,
                                            const char* iconFont,
                                            const char* filesDir) {
    auto& bridge = AndroidBridge::instance();
