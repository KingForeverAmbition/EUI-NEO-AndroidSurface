...
extern "C" void eui_android_request_exit(void);
...
namespace app
{
    ...
    namespace
    {
        ...
        // 控件
        bool g_touchPassthrough = false;
        bool g_switchOn = true;
        bool g_switchDark = false;
        bool g_checkA = true;
        bool g_checkB = false;
        bool g_checkC = false;
        int g_radioSelect = 2;        // 0=浅色, 1=深色, 2=自动（跟随系统）
        bool g_systemDarkMode = true; // 由 Java 层写入；首次读取配置前默认深色
    ...
    {
        // 主题
        // g_radioSelect 同时充当主题模式：
        //   0 = 浅色（强制）
        //   1 = 深色（强制）
        //   2 = 自动（跟随系统 uiMode，由 Java 通过 nativeSetDarkMode 上报）
        bool effectiveDark()
        {
            if (g_radioSelect == 0)
                return false;
        ...
        core::Color textSec()
        {
            return components::theme::withAlpha(themeColors().text, 0.70f);
        }

        core::Color textDim()
        {
            return components::theme::withAlpha(themeColors().text, 0.55f);
        }
        ...
        core::Color dangerC()
        {
            return {0.95f, 0.30f, 0.35f, 1.0f};
        }

        core::Color btnHover(core::Color c)
        {
            return core::mixColor(c, {1.0f, 1.0f, 1.0f, c.a}, 0.16f);
        }

        core::Color btnPress(core::Color c)
        {
            return core::mixColor(c, {0.04f, 0.05f, 0.08f, c.a}, 0.30f);
        }
        ...
        void cardSection(core::dsl::Ui &ui, const std::string &id, float width, float height, Body &&body, int zIndex = 0)
        {
        ...
        void sectionTitle(core::dsl::Ui &ui, const std::string &id, const std::string &title, const std::string &subtitle, float width)
        {
            ui.column(id + ".hdr")
                .width(width)
        ...
        void sidebarItem(core::dsl::Ui &ui, const std::string &id, const std::string &label, unsigned int icon, int page)
        {
            ...
            ui.rect(id + ".bg")
                ...
                .onClick([page] {
                    g_selectedPage = page;
                    g_dropdownOpen = false; // 切换页面时重置下拉框的瞬态状态
                    saveSettings();
                })
                .build();
        ...
        void pageControls(core::dsl::Ui &ui, float w)
        {
            ...
            // 退出程序
            sectionTitle(ui, "exit", "程序控制", "退出悬浮窗", cardW);
            cardSection(ui, "exit.card", cardW, 80.0f, [&]
                        { components::button(ui, "btn.exit")
                              .size(innerW, 44.0f)
                              .text("退出程序")
                              .colors({0.85f, 0.22f, 0.22f, 1.f},
                                      {0.92f, 0.30f, 0.30f, 1.f},
                                      {0.70f, 0.15f, 0.15f, 1.f})
                              .radius(10.0f)
                              .onClick([]
                                       { eui_android_request_exit(); })
                              .build(); });
        ...
        void pagePickers(core::dsl::Ui &ui, float w)
        {
            ...
            cardSection(ui, "dd", cardW, 80.0f, [&]
                        { components::dropdown(ui, "dd.country").size(innerW, 44.0f).theme(themeColors()).items({"中国 China", "美国 USA", "日本 Japan", "韩国 Korea", "德国 Germany"}).selected(g_dropdown).open(g_dropdownOpen).onChange([](int idx)
                                                                                                                                                                                                                                           {
                g_dropdown = idx;
                saveSettings(); })
        ...
        void pageCharts(core::dsl::Ui &ui, float w)
        {
            ...
            for (int i = 0; i < 7; ++i)
            {
                line[i] = 0.50f + 0.30f * std::sin(t + i * 0.55f) + 0.12f * std::cos(t * 0.6f + i * 1.1f);
            }
            std::vector<float> bar(6);
            for (int i = 0; i < 6; ++i)
            {
                bar[i] = 0.45f + 0.28f * std::sin(t * 0.8f + i * 0.9f + 1.2f) + 0.12f * std::cos(t * 1.4f + i * 0.3f);
            }
            // 归一化
            ...
            cardSection(ui, "ln", cardW, 200.0f, [&]
                        { components::lineChart(ui, "ch.line").size(innerW, 160.0f).values(line).theme(themeColors()).build(); });

            sectionTitle(ui, "br", "柱状图 Bar Chart", "", cardW);
            cardSection(ui, "br", cardW, 200.0f, [&]
                        { components::barChart(ui, "ch.bar").size(innerW, 160.0f).values(bar).theme(themeColors()).build(); });

            sectionTitle(ui, "pi", "饼图 Pie Chart", "", cardW);
            cardSection(ui, "pi", cardW, 220.0f, [&]
                        { components::pieChart(ui, "ch.pie").size(180.0f, 180.0f).values(pie).theme(themeColors()).build(); });

            ui.stack("ch.tick").size(0.0f, 0.0f).onTimer(0.10f, []
                                                         { ++g_tick; })
                .build();
        }
        ...
        void pageClock(core::dsl::Ui &ui, float w)
        {
            ...
            char dateBuf[32];
            std::snprintf(dateBuf, sizeof(dateBuf), "%04d-%02d-%02d", local.tm_year + 1900, local.tm_mon + 1, local.tm_mday);
        ...
        components::text(ui, "ck.date")
            ...
            ui.stack("ck.tick").size(0.0f, 0.0f).onTimer(0.25f, []
                                                         { ++g_tick; })
                .build();
        }
    ...
    const DslAppConfig &dslAppConfig()
    {
        static DslAppConfig config = DslAppConfig{}
                                         ...
                                         .pageId("main")
                                         .clearColor(appBg())
                                         .windowSize(native_window_screen_x, native_window_screen_y)
                                         .fps(120.0)
    ...
    void compose(core::dsl::Ui &ui, const core::dsl::Screen &screen)
    {
        ...
        components::dialog(ui, "dlg.confirm")
            ...
            .onPrimary([]
                       { g_dialogOpen = false; showToast("操作已确认"); })
            .onSecondary([]
                         { g_dialogOpen = false; showToast("操作已取消"); })
            .onClose([]
                     { g_dialogOpen = false; })
            .build();
        ...
        components::datepicker(ui, "dp.cal")
            ...
            .onChange([](int y, int m, int d)
                      { g_year = y; g_month = m; g_day = d; saveSettings(); })
            .onOpenChange([](bool open)
                          { g_datePickerOpen = open; })
            .build();
        ...
        components::timepicker(ui, "tp.clock")
            ...
            .onChange([](int h, int m)
                      { g_hour = h; g_minute = m; saveSettings(); })
            .onOpenChange([](bool open)
                          { g_timePickerOpen = open; })
            .build();
        ...
        components::colorpicker(ui, "cp.color")
            ...
            .value(g_pickedColor)
            .onChange([](const core::Color &c)
                      { g_pickedColor = c; saveSettings(); })
            .onOpenChange([](bool open)
                          { g_colorPickerOpen = open; })
            .build();
